...
auth.post(
  ...
  async (c) => {
    ...
    const existingAgent = await db
      .select()
      .from(agents)
      .where(eq(agents.email, body.email))
      .get();
    ...
    if (body.workspaceName && body.workspaceSubdomain) {
      ...
      const existingWorkspace = await db
        .select()
        .from(workspaces)
        .where(eq(workspaces.subdomain, body.workspaceSubdomain))
        .get();
...
auth.get('/me', authMiddleware, async (c) => {
  ...
  const agent = await db
    .select()
    .from(agents)
    .where(eq(agents.id, agentContext.id))
    .get();
...
auth.get('/microsoft/callback', zValidator('query', MicrosoftCallbackSchema), async (c) => {
  ...
  let agent = await db
    .select()
    .from(agents)
    .where(eq(agents.microsoftId, microsoftUser.id))
    .get();
