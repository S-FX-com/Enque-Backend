...
class MicrosoftGraphService:
    def __init__(self, db: AsyncSession):
        self.db = db
        self.integration = None  # Will be initialized in an async method
    ...
    async def initialize(self):
        self.integration = await self._get_active_integration()
        self.auth_service.integration = self.integration
        if self.integration:
            pass
    ...
    async def _get_active_integration(self) -> Optional[MicrosoftIntegration]:
        stmt = select(MicrosoftIntegration).where(MicrosoftIntegration.is_active == True).order_by(MicrosoftIntegration.id.desc())
        result = await self.db.execute(stmt)
        integrations = result.scalars().all()

        if len(integrations) > 1:
            logger.warning(f"Found {len(integrations)} active Microsoft integrations. Using the most recent one (ID: {integrations[0].id})")
    ...
    async def exchange_code_for_token(self, code: str, redirect_uri: str, state: Optional[str] = None) -> MicrosoftToken:
        return await self.auth_service.exchange_code_for_token(code, redirect_uri, state)

    async def sync_emails(self, sync_config: EmailSyncConfig):
        return await self.email_service.sync_emails(sync_config, self._get_user_email_for_sync)
    ...
    async def _get_user_email_for_sync(self, config: EmailSyncConfig = None) -> Tuple[Optional[str], Optional[MicrosoftToken]]:
        token: Optional[MicrosoftToken] = None
        mailbox_email: Optional[str] = None
        if config:
            result = await self.db.execute(select(MailboxConnection).filter(MailboxConnection.id == config.mailbox_connection_id))
            ...
            if not token:
                ...
                expired_refreshable_token = result.scalars().first()

                if expired_refreshable_token:
                    logger.info(f"Found refreshable token ID: {expired_refreshable_token.id} for MailboxConnection ID: {mailbox.id}. Attempting async refresh.")
                    try:
                        token = await self.auth_service.refresh_token_async(expired_refreshable_token)
                        logger.info(f"Successfully refreshed token ID: {token.id} asynchronously for MailboxConnection ID: {mailbox.id}.")
                    except HTTPException as e:
                        logger.error(f"Async refresh failed for token ID {expired_refreshable_token.id} (MailboxConnection ID: {mailbox.id}): {e.detail}")
                        token = None
                    except Exception as e:
                        logger.error(f"Unexpected error during async refresh for token ID {expired_refreshable_token.id} (MailboxConnection ID: {mailbox.id}): {str(e)}", exc_info=True)
                        token = None
                else:
            ...
            token = await self.get_most_recent_valid_token()
            if not token:
                logger.warning("No recent valid token found across all mailboxes (via get_most_recent_valid_token).")
                return None, None
            result = await self.db.execute(select(MailboxConnection).filter(MailboxConnection.id == token.mailbox_connection_id))
            mailbox = result.scalars().first()
            if not mailbox:
    ...
    async def get_most_recent_valid_token(self) -> Optional[MicrosoftToken]:
        result = await self.db.execute(select(MicrosoftToken).filter(MicrosoftToken.expires_at > datetime.utcnow()).order_by(MicrosoftToken.created_at.desc()))
        token = result.scalars().first()
        if token:
            return token
        ...
        if expired_refreshable_token:
            logger.info(f"No active token found by get_most_recent_valid_token. Found refreshable token ID: {expired_refreshable_token.id}. Attempting async refresh.")
            try:
                refreshed_token = await self.auth_service.refresh_token_async(expired_refreshable_token)
                logger.info(f"Successfully refreshed token ID: {refreshed_token.id} asynchronously via get_most_recent_valid_token.")
                return refreshed_token
            except HTTPException as e:
                logger.error(f"Async refresh failed in get_most_recent_valid_token for token ID {expired_refreshable_token.id}: {e.detail}")
            except Exception as e:
                logger.error(f"Unexpected error during async refresh in get_most_recent_valid_token for token ID {expired_refreshable_token.id}: {str(e)}", exc_info=True)
...
async def get_microsoft_service(db: AsyncSession) -> MicrosoftGraphService:
    service = MicrosoftGraphService(db)
    await service.initialize()
    return service

async def mark_email_as_read_by_task_id(db: AsyncSession, task_id: int) -> bool:
    try:
        result = await db.execute(select(Task).filter(Task.id == task_id))
        task = result.scalars().first()
        if not task or task.status != "Open": return False
        result = await db.execute(select(EmailTicketMapping).filter(EmailTicketMapping.ticket_id == task_id))
        ...
        service = await get_microsoft_service(db)
        ...
        result = await db.execute(select(MailboxConnection).filter(MailboxConnection.id == task.mailbox_connection_id))
        mailbox = result.scalars().first()
        if not mailbox: logger.error(f"MailboxConnection not found for ID {task.mailbox_connection_id}"); return False
        result = await service.graph_client.mark_email_as_read(app_token, mailbox.email, mapping.email_id)
