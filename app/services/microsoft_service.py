...
from app.schemas.microsoft import EmailData, EmailAddress, EmailAttachment, MicrosoftTokenCreate, EmailTicketMappingCreate, TokenResponse
...
class MicrosoftGraphService:
    ...
    def sync_emails(self, sync_config: EmailSyncConfig):
        ...
        try:
            logger.info(f"Syncing emails for config #{sync_config.id} from folder '{sync_config.folder_name}'")
            ...
            logger.info(f"Found {len(emails_data)} emails for sync configuration #{sync_config.id}")
            ...
            for email_data in emails_data:
                try:
                    ...
                    if existing_mapping:
                        logger.debug(f"Email {email_id} has already been processed as ticket #{existing_mapping.task_id}")
                        continue
                    ...
                    if task:
                        # Create email to ticket mapping
            ...
            self.db.commit()
            logger.info(f"Created {len(created_tasks)} new tickets from emails")
            
            # Update last sync time
            sync_config.last_sync_time = datetime.utcnow()
            self.db.commit()
            logger.info(f"Sync completed for config #{sync_config.id}")
            
            return created_tasks
            
        except Exception as e:
            logger.error(f"Error syncing emails for config #{sync_config.id}: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Failed to fetch emails for {user_email}: {str(e)}"
            )
    
    def _create_task_from_email(self, email: EmailData, config: EmailSyncConfig) -> Task:
        ...
        user = get_or_create_user(self.db, user_email, email.sender.name if email.sender else "Unknown")
        ...
        if len(email.body_content or "") > 10:  # Only add if there's substantial content
            comment = Comment(
                task_id=task.id,
                user_id=sent_from_agent.id,  # Comment comes from the system/agent
    ...
    def _get_user_email_for_sync(self):
        ...
        # Primero, intentamos renovar todos los tokens
        ...
        # Buscar un agente con un email válido de Microsoft
        agent = self.db.query(Agent).filter(
            Agent.email.like("%@%")  # Email básicamente válido
        ).filter(
            ~Agent.email.like("%example.com")  # No usar emails de ejemplo
        ).first()
        
        if agent and agent.email:
            logger.info(f"Usando email del agente encontrado: {agent.email}")
            return agent.email
            
        # Si todo falla, lanzar una excepción en lugar de continuar con un email inválido
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    def get_most_recent_valid_token(self) -> Optional[MicrosoftToken]:
        ...
        if token.expires_at > datetime.utcnow():
            logger.info(f"Usando token vigente (ID: {token.id})")
            return token
            
        # If it's expired, we try to renew it
        try:
            logger.info(f"Intentando renovar token expirado (ID: {token.id})")
    ...
    def check_and_refresh_all_tokens(self) -> None:
        ...
        if settings.CLEANUP_OLD_TOKENS:
            try:
                ...
                if old_tokens:
                    self.db.commit()
                    logger.info(f"Se eliminaron {len(old_tokens)} tokens antiguos")
            except Exception as e:
                logger.warning(f"Error al limpiar tokens antiguos: {str(e)}")
