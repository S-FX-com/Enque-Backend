...
async def read_companies(
    ...
    """
    companies = db.query(Company).order_by(Company.name).offset(skip).limit(limit).all()
    return companies
...
async def read_company_users(
    ...
    users = db.query(User).filter(User.company_id == company_id).order_by(User.name).offset(skip).limit(limit).all()
    return users 