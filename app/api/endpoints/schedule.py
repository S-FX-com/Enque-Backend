...
@router.post("/tasks/{task_id}/comments/schedule")  # <-- Path is the first argument
async def create_scheduled_comment(
    task_id: int,
    ...
    scheduled_task.add_job(
        create_comment(task_id, comment_in, request, background_tasks, db, current_user),
        'cron',
        day=date.day,
        month=date.month,
        year=date.year,
