...
from app.services import global_signature_service as service

router = APIRouter()


@router.post("/", response_model=GlobalSignature)
async def create_global_signature(
    *,
    ...
    existing_signature = await service.get_by_workspace_id(
...
async def get_global_signature(
    *,
    db: AsyncSession = Depends(get_db),
    current_agent: Agent = Depends(get_current_active_agent),
    workspace_id: int,
    ...
    global_signature = await service.get_by_workspace_id(
...
async def get_enabled_global_signature(
    *,
    db: AsyncSession = Depends(get_db),
    current_agent: Agent = Depends(get_current_active_agent),
    workspace_id: int,
    ...
    global_signature = await service.get_enabled_by_workspace_id(
        db=db, workspace_id=workspace_id
    )
...
async def update_global_signature(
    *,
    db: AsyncSession = Depends(get_db),
    current_agent: Agent = Depends(get_current_active_agent),
    workspace_id: int,
    ...
    global_signature = await service.update(
...
async def delete_global_signature(
    *,
    db: AsyncSession = Depends(get_db),
    current_agent: Agent = Depends(get_current_active_agent),
    workspace_id: int,
    ...
    await service.delete(db=db, db_obj=global_signature)
